...
2. Create directory structure:
   mkdir -p src/{app_name}/components
   mkdir -p src/{app_name}/templatetags
   mkdir -p src/{app_name}/tests
   mkdir -p src/templates/{app_name}/partials
   mkdir -p src/templates/{app_name}/components
   ...
   touch src/{app_name}/components/__init__.py
   touch src/{app_name}/templatetags/__init__.py
...
5. Create templatetags/{app_name}_components.py with:
   ```python
   from django import template
   register = template.Library()
   ```

6. Register in `src/config/settings.py` INSTALLED_APPS:
   Add `"{app_name}"` ONLY IF not already present in the list.

7. Register in `src/config/urls.py`:
...
10. Print summary:
    "Created app src/{app_name}/ with components/, templatetags/, templates/{app_name}/components/"
