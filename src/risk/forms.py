...
class HazardForm(forms.ModelForm):
    class Meta:
        model = Hazard
        fields = [
            "title", "description", "severity", "probability",
            "mitigation", "mitigation_status", "due_date",
        ]
        widgets = {
            "title": forms.TextInput(attrs={
                "class": "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-orange-500 focus:border-orange-500",
            ...
            "mitigation_status": forms.Select(attrs={
                "class": "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm",
            }),
...
class ProtectiveMeasureForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            "measure_type", "description", "specification",
            "norm_reference", "due_date", "status",
        ]
        widgets = {
            "measure_type": forms.Select(attrs={
                "class": "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm",
            }),
            "description": forms.Textarea(attrs={
                "class": "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm",
            ...
            "norm_reference": forms.TextInput(attrs={
                "class": "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm",
...
class SubstitutionCheckForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "implementation_deadline": forms.DateInput(attrs={
                "class": "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm",
                "type": "date",
            }),
        }
