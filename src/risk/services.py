...
from bfagent_core import get_context, emit_audit_event, emit_outbox_event
...
def create_assessment(cmd: CreateAssessmentCmd) -> Assessment:
    ...
    emit_outbox_event(
        ...
        payload={"assessment_id": str(assessment.id), "title": assessment.title},
    )
...
def approve_assessment(cmd: ApproveAssessmentCmd) -> Assessment:
    ...
    assessment = Assessment.objects.get(id=cmd.assessment_id)
...
async def analyze_hazard_with_ai(hazard_description: str) -> str:
    ...
    result = await framework.execute(
        system_prompt="""Du bist ein Experte für Gefährdungsbeurteilungen und Arbeitssicherheit.
Analysiere die beschriebene Gefährdung und gib Empfehlungen für Schutzmaßnahmen.""",
        user_prompt="""Analysiere folgende Gefährdung und schlage geeignete Schutzmaßnahmen vor:
