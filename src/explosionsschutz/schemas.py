...
class ZoneExtent(BaseModel):
    ...
    # Für sphere
    radius_m: float | None = Field(None, ge=0, description="Radius in Metern")

    # Für cylinder
    diameter_m: float | None = Field(None, ge=0, description="Durchmesser in Metern")
    height_m: float | None = Field(None, ge=0, description="Höhe in Metern")

    # Für box
    length_m: float | None = Field(None, ge=0, description="Länge in Metern")
    width_m: float | None = Field(None, ge=0, description="Breite in Metern")
    depth_m: float | None = Field(None, ge=0, description="Tiefe in Metern")

    # Für alle
    origin_description: str | None = Field(None, description="Beschreibung des Ursprungspunkts")
    reference_drawing: str | None = Field(None, description="Referenz auf technische Zeichnung")

    # Für custom
    custom_description: str | None = Field(
        None, description="Freitextbeschreibung für komplexe Geometrien"
    )
